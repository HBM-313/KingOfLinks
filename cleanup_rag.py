#!/usr/bin/env python3
"""Cleanup script for RAG folder - fix all data quality issues."""
...
def fix_editorial_sn_block(block, soup):
    """Fix EDITORIAL_SN: parse scholar/book from bt, update sn+bt, add editorial note."""
    global changes
    ...
    # Pattern C: bt is a known book title
    ...
    # Pattern B: bt has dash → "scholar - book [- chapter...]"
        ...
        if m:
            ...
            # Strip trailing chapter keywords from book name
            ch_m = CHAPTER_SPLIT_RE.search(rest)
            book = rest[:ch_m.start()].strip() if ch_m else rest
            # Remove trailing dash or keyword suffix
            book = re.sub(r'\s*-+\s*\w*$', '', book).strip() if '-' in book else book
        else:
            scholar = bt_text
            book = bt_text

    # Pattern A: bt=scholar, book from first chapter-info
    else:
        scholar = bt_text
        if ci_texts:
...
def fix_sn_number_bt_text(block, soup, scholar, book, hadith_num):
    """Fix inversion: sn=number, bt=hadith text → move bt to hadith-text."""
    global changes
    sn = block.select_one('.scholar-name')
    bt = block.select_one('.book-title')
    if not bt:
...
def process_file(path):
    ...
    if path == 'RAG/1Ftawi.html':
        # b2: sn='‏6970', bt=full hadith text (inversion from صحيح البخاري)
        b2 = blocks[1]
        sn2 = b2.select_one('.scholar-name')
        if sn2:
            num2 = re.sub(r'[^\d]', '', sn2.get_text())
            if num2 == '6970' and not b2.select_one('.hadith-text'):
                fix_sn_number_bt_text(b2, soup, 'البخاري', 'صحيح البخاري', num2)

        # b3: sn='‏6971', bt=full hadith text (inversion from صحيح البخاري)
        b3 = blocks[2]
        sn3 = b3.select_one('.scholar-name')
        if sn3:
            num3 = re.sub(r'[^\d]', '', sn3.get_text())
            if num3 == '6971' and not b3.select_one('.hadith-text'):
                fix_sn_number_bt_text(b3, soup, 'البخاري', 'صحيح البخاري', num3)
...
def main():
    global changes
    paths = sorted(
        glob.glob('RAG/*.html') + glob.glob('RAG/**/*.html', recursive=True)
    )
    seen = set()
